package fr.dreamin.dreamintools.api.task;
...
public class Task {
  ...
  public void cancelAndRunImmediately() {
    ...
    if (this.bukkitRunnable != null) {
      this.bukkitRunnable.run();
    }

    if (this.defaultRunnable != null) {
      this.defaultRunnable.run();
    }
    ...
    if (this.bukkitRunnable != null) {
      if (this.async) {
        if (this.repeat > 0) {
          ...
          this.task = this.bukkitRunnable.runTaskAsynchronously(this.plugin);
        }
      } else if (this.repeat > 0) {
        this.task = this.bukkitRunnable.runTaskTimer(this.plugin, (long)this.delay, (long)this.repeat);
      } else if (this.delay > 0) {
        this.task = this.bukkitRunnable.runTaskLater(this.plugin, (long)this.delay);
      } else {
        this.task = this.bukkitRunnable.runTask(this.plugin);
      }
    } else {
      if (this.defaultRunnable == null) {
      ...
      if (this.async) {
        if (this.repeat > 0) {
          ...
          this.task = scheduler.runTaskAsynchronously(this.plugin, this.defaultRunnable);
        }
      } else if (this.repeat > 0) {
        this.task = scheduler.runTaskTimer(this.plugin, this.defaultRunnable, (long)this.delay, (long)this.repeat);
      } else if (this.delay > 0) {
        this.task = scheduler.runTaskLater(this.plugin, this.defaultRunnable, (long)this.delay);
      } else {
        this.task = scheduler.runTask(this.plugin, this.defaultRunnable);
      }
    }
