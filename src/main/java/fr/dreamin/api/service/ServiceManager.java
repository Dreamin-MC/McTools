...
public class ServiceManager {
  ...
  public Service loadService(Class<? extends Service> target) {
    try {
      ...
      if (isServiceEnabled(target)) return serviceMap.get(target);
      ...
      if (i == null) i = target.getConstructor().newInstance();
      ...
      for (Field field : target.getDeclaredFields()) {
        ...
        if (hasAnnotation) {
          ...
          if (v != null) {
            ...
            if (v == null) System.out.println("Field " + field.getName() + " in " + target.getSimpleName() + " is null, this is probably a bug");
            else System.out.println("WARNING! field " + field.getName() + " in " + target.getSimpleName() + " doesn't have the inject annotation, but it was resolved as " + v.getClass().getName());
          }
        }
      }
      ...
      McTools.getInstance().getLogger().warning("Failed to load service: " + target.getName());
      throw e;
    }
  }
  ...
  public Object resolve(Class<?> type) {
    // its depended upon
    if (Service.class.isAssignableFrom(type)) return loadService((Class<? extends Service>) type);
    else if (type == McTools.class) return McTools.getInstance();
    else {
      if (otherInjectables.containsKey(type)) return otherInjectables.get(type);
        ...
        for (Map.Entry<Class<?>, Object> entry : otherInjectables.entrySet()) {
          ...
          if (type.isAssignableFrom(ci)) return vi;
        }
      } else {
        // last option, it extends, somewhere
        for (Map.Entry<Class<?>, Object> entry : otherInjectables.entrySet()) {
          Class<?> c = entry.getKey();
          if (c.isAssignableFrom(type)) return entry.getValue();
        }
      }
    }
    return null;
  ...
  public void registerDependency(Class<?> key, Object value) {
    if (Service.class.isAssignableFrom(key)) serviceMap.put((Class<? extends Service>) key, (Service) value);
