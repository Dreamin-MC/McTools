 ...
 * @version 2.0.1
 */
public abstract class DreaminBoardBase<T> {
  ...
  // Scoreboard packets
  private static final DreaminReflection.PacketConstructor PACKET_SB_OBJ;
  private static final DreaminReflection.PacketConstructor PACKET_SB_DISPLAY_OBJ;
  private static final DreaminReflection.PacketConstructor PACKET_SB_TEAM;
  private static final DreaminReflection.PacketConstructor PACKET_SB_SERIALIZABLE_TEAM;
  ...
  static {
    try {
      ...
      if (DreaminReflection.isRepackaged()) {
        VERSION_TYPE = VersionType.V1_17;
      } else if (DreaminReflection.nmsOptionalClass(null, "ScoreboardServer$Action").isPresent()
        || DreaminReflection.nmsOptionalClass(null, "ServerScoreboard$Method").isPresent()) {
        VERSION_TYPE = VersionType.V1_13;
      } else if (DreaminReflection.nmsOptionalClass(null, "IScoreboardCriteria$EnumScoreboardHealthDisplay").isPresent()
        || DreaminReflection.nmsOptionalClass(null, "ObjectiveCriteria$RenderType").isPresent()) {
      ...
      Class<?> craftPlayerClass = DreaminReflection.obcClass("entity.CraftPlayer");
      Class<?> entityPlayerClass = DreaminReflection.nmsClass("server.level", "EntityPlayer", "ServerPlayer");
      Class<?> playerConnectionClass = DreaminReflection.nmsClass("server.network", "PlayerConnection", "ServerGamePacketListenerImpl");
      Class<?> packetClass = DreaminReflection.nmsClass("network.protocol", "Packet");
      Class<?> packetSbObjClass = DreaminReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardObjective", "ClientboundSetObjectivePacket");
      Class<?> packetSbDisplayObjClass = DreaminReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardDisplayObjective", "ClientboundSetDisplayObjectivePacket");
      Class<?> packetSbScoreClass = DreaminReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardScore", "ClientboundSetScorePacket");
      Class<?> packetSbTeamClass = DreaminReflection.nmsClass(gameProtocolPackage, "PacketPlayOutScoreboardTeam", "ClientboundSetPlayerTeamPacket");
      Class<?> sbTeamClass = VersionType.V1_17.isHigherOrEqual()
        ? DreaminReflection.innerClass(packetSbTeamClass, innerClass -> !innerClass.isEnum()) : null;
      ...
      Optional<Class<?>> displaySlotEnum = DreaminReflection.nmsOptionalClass("world.scores", "DisplaySlot");
      CHAT_COMPONENT_CLASS = DreaminReflection.nmsClass("network.chat", "IChatBaseComponent","Component");
      CHAT_FORMAT_ENUM = DreaminReflection.nmsClass(null, "EnumChatFormat", "ChatFormatting");
      DISPLAY_SLOT_TYPE = displaySlotEnum.orElse(int.class);
      RESET_FORMATTING = DreaminReflection.enumValueOf(CHAT_FORMAT_ENUM, "RESET", 21);
      SIDEBAR_DISPLAY_SLOT = displaySlotEnum.isPresent() ? DreaminReflection.enumValueOf(DISPLAY_SLOT_TYPE, "SIDEBAR", 1) : 1;
      PLAYER_GET_HANDLE = lookup.findVirtual(craftPlayerClass, "getHandle", MethodType.methodType(entityPlayerClass));
      PLAYER_CONNECTION = lookup.unreflectGetter(playerConnectionField);
      SEND_PACKET = lookup.unreflect(sendPacketMethod);
      PACKET_SB_OBJ = DreaminReflection.findPacketConstructor(packetSbObjClass, lookup);
      PACKET_SB_DISPLAY_OBJ = DreaminReflection.findPacketConstructor(packetSbDisplayObjClass, lookup);

      Optional<Class<?>> numberFormat = DreaminReflection.nmsOptionalClass("network.chat.numbers", "NumberFormat");
      ...
      if (numberFormat.isPresent()) { // 1.20.3
        Class<?> blankFormatClass = DreaminReflection.nmsClass("network.chat.numbers", "BlankFormat");
        Class<?> fixedFormatClass = DreaminReflection.nmsClass("network.chat.numbers", "FixedFormat");
        Class<?> resetScoreClass = DreaminReflection.nmsClass(gameProtocolPackage, "ClientboundResetScorePacket");
        ...
        Optional<MethodHandle> optionalScorePacket = DreaminReflection.optionalConstructor(packetSbScoreClass, lookup, scoreTypeOptional);
        ...
        Class<?> enumSbAction = DreaminReflection.nmsClass("server", "ScoreboardServer$Action", "ServerScoreboard$Method");
      ...
      PACKET_SB_TEAM = DreaminReflection.findPacketConstructor(packetSbTeamClass, lookup);
      PACKET_SB_SERIALIZABLE_TEAM = sbTeamClass == null ? null : DreaminReflection.findPacketConstructor(sbTeamClass, lookup);
      ...
      if (VersionType.V1_8.isHigherOrEqual()) {
        ...
        ENUM_SB_HEALTH_DISPLAY = DreaminReflection.nmsClass("world.scores.criteria", "IScoreboardCriteria$EnumScoreboardHealthDisplay", "ObjectiveCriteria$RenderType");
        ENUM_SB_ACTION = DreaminReflection.nmsClass("server", enumSbActionClass, "ServerScoreboard$Method");
        ENUM_SB_HEALTH_DISPLAY_INTEGER = DreaminReflection.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER", 0);
        ENUM_SB_ACTION_CHANGE = DreaminReflection.enumValueOf(ENUM_SB_ACTION, "CHANGE", 0);
        ENUM_SB_ACTION_REMOVE = DreaminReflection.enumValueOf(ENUM_SB_ACTION, "REMOVE", 1);
      } else {
   ...
   */
  protected DreaminBoardBase(Player player) {
