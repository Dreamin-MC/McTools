    ...
    new BukkitRunnable() {
      ...
      public void run() {
        ...
        for (int degree = 0; degree < 360; degree += 15) {
          ...
          if (showAll) player.getWorld().spawnParticle(Particle.DUST, player.getLocation().add(new Vector(x, 0, z)), 0, new Particle.DustOptions(Color.RED, 1));
          else player.spawnParticle(Particle.DUST, player.getLocation().add(new Vector(x, 0, z)), 0, new Particle.DustOptions(Color.RED, 1));
        }
    ...
    new BukkitRunnable() {
      ...
      public void run() {
        ...
        for (int degree = 0; degree < 360; degree += 15) {
          ...
          if (showAll) player.getWorld().spawnParticle(Particle.DUST, player.getLocation().add(new Vector(x, 0, z)), 0, new Particle.DustOptions(Color.GREEN, 1));
          else player.spawnParticle(Particle.DUST, player.getLocation().add(new Vector(x, 0, z)), 0, new Particle.DustOptions(Color.GREEN, 1));
        }
        radius += radiusIncrement;
        elapsedMicroSeconds += 0.1;
      }
        ...
        for (int y = y1; y <= y2; y++) {
          for (int z = z1; z <= z2; z++) {
            if (x == x1 || x == x2 || y == y1 || y == y2 || z == z1 || z == z2) {
              if (player.getLocation().distance(new Location(world,x, y, z)) <= 30) player.spawnParticle(Particle.HAPPY_VILLAGER, x + 0.5, y + 0.5, z + 0.5, 1, 0, 0, 0, 0);
            }
          }
        }
      }
      ...
      for (int y = y1; y <= y2; y++) {
        for (int z = z1; z <= z2; z++) {
          if (x == x1 || x == x2 || y == y1 || y == y2 || z == z1 || z == z2) {
            ...
            if (player.getLocation().distance(loc) <= 30) player.spawnParticle(Particle.DUST, loc.add(0.5, 0.5, 0.5), 1, 0, 0, 0, 0, color);
          }
        }
      }
    }
  ...
  public static void showCuboid(Player player, Location coin1, Location coin2) {
    ...
    for (int x = x1; x <= x2; x++) {
      for (int y = y1; y <= y2; y++) {
        for (int z = z1; z <= z2; z++) {
          if (x == x1 || x == x2 || y == y1 || y == y2 || z == z1 || z == z2) {
            if (player.getLocation().distance(new Location(world,x, y, z)) <= 50) player.spawnParticle(Particle.HAPPY_VILLAGER, x + 0.5, y + 0.5, z + 0.5, 1, 0, 0, 0, 0);
          }
        }
      }
    }
