...
public enum PaginationType {
  PAGE {
    public void setPagination(Player player, PaginationManager paginationManager, GuiBuilder menu, GuiItems items) {
      ...
      if (page > 1) items.create(paginationManager.getPrevious(), paginationManager.getSlotPrevious());
      if (page < totalPages) items.create(paginationManager.getNext(), paginationManager.getSlotNext());
    ...
    public void setNext(Player player, PaginationManager paginationManager, Class<? extends GuiBuilder> gClass) {
      ...
      if (paginationManager.isSync()) McTools.getService(GuiManager.class).getGuiConfig().getGuiPageManager().addPageForAll(gClass.getSimpleName());
    ...
    public void setPrevious(Player player, PaginationManager paginationManager, Class<? extends GuiBuilder> gClass) {
      if (paginationManager.isSync()) McTools.getService(GuiManager.class).getGuiConfig().getGuiPageManager().removePageForAll(gClass.getSimpleName());
  ...
  LINE {
    ...
    public void setPagination(Player player, PaginationManager paginationManager, GuiBuilder menu, GuiItems items) {
      ...
      if (indexStart > 1) items.create(paginationManager.getPrevious(), paginationManager.getSlotPrevious());
      if (indexStart - 1 + itemsPerPage < paginationManager.getItems().size()) items.create(paginationManager.getNext(), paginationManager.getSlotNext());
