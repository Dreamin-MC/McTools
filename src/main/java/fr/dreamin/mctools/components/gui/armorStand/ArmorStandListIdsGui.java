...
public class ArmorStandListIdsGui implements GuiBuilder {

  @Override
  public String name(MTPlayer mtPlayer) {
    return McTools.getCodex().isResourcePack() ? CustomChatColor.WHITE.getColorWithText(PictureGui.GENERIC_45.getName()) : "Id Gui";
  }
  ...
  public void onClick(MTPlayer mtPlayer, Inventory inv, ItemStack current, int slot, ClickType action, int indexPagination) {
    ...
    switch (slot) {
      case 37:
        mtPlayer.getArmorStandManager().removeAllArmorStandId();
        ...
        break;
      case 40:
        mtPlayer.getArmorStandManager().dispawnAllArmorStandId();
        mtPlayer.getPlayer().closeInventory();
        mtPlayer.sendMsg(LangMsg.GENERAL_HAVEALLARMORSTAND, LangMsg.GENERAL_DESTROY, LangMsg.GENERAL_SELECTED);
        break;
      case 43:
        mtPlayer.getArmorStandManager().addAllArmorStandSelected(mtPlayer.getArmorStandManager().getArmorStandId());
        mtPlayer.getArmorStandManager().removeAllArmorStandId();
      ...
      default:
        ...
        if (indexPagination > -1) {
          ArmorStand armorStand = mtPlayer.getArmorStandManager().getArmorStandId().get(indexPagination);
          if (action.equals(ClickType.LEFT)) {
            mtPlayer.getArmorStandManager().removeArmorStandId(armorStand);
